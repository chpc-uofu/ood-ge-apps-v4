...
apptainer run --nv -e --fakeroot --env HF_HOME=/root/.cache/ --bind ${SCRATCH_DIR}:/root/.cache  /uufs/chpc.utah.edu/sys/installdir/r8/vllm/vllm-0.8.5p1.sif --host=localhost --port=$nimport --model=$MODEL --hf-token=$HUGGINGFACE_TOKEN --download_dir=$SCRATCH_DIR
