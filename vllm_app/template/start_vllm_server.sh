...
apptainer run --nv -e --fakeroot --env HF_HOME=/root/.cache/ --bind ${SCRATCH_DIR}:/root/.cache  /uufs/chpc.utah.edu/sys/installdir/r8/vllm/vllm_current_cuda_sif_link $MODEL  --host=localhost --port=$nimport --hf-token=$HUGGINGFACE_TOKEN --download_dir=$SCRATCH_DIR --task $TASK_CHOICE
